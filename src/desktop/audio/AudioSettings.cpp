...
QJsonArray parseAudioDevices(const QByteArray &text) {
  ...
  for (const auto &line : QString::fromUtf8(text).split('\n')) {
    ...
    if (match.hasMatch())
      devices.append(QJsonObject{{"id", match.captured(2).toInt()},
...
QJsonObject parseAudioVolume(const QByteArray &text) {
  const auto match =
      QRegularExpression("^Volume: ([0-9]+(?:\\.[0-9]+)?)( \\[MUTED\\])?\\s*$")
  ...
  return {
      {"available", true},
      {"volume", std::clamp(static_cast<int>(std::round(value * 100)), 0, 100)},
...
QStringList audioCommand(const QJsonObject &change) {
  ...
  const QString target =
      device == "output" ? "@DEFAULT_AUDIO_SINK@" : "@DEFAULT_AUDIO_SOURCE@";
  if (change.contains("mute") && change.value("mute").isBool())
    return {"set-mute", target, change.value("mute").toBool() ? "1" : "0"};
  const auto value = change.value("volume");
  const double number = value.toDouble(-1);
  if (!value.isDouble() || !std::isfinite(number) || number < 0 ||
      number > 100 || std::floor(number) != number)
    return {};
  return {"set-volume", target, QString::number(static_cast<int>(number)) + "%",
          "--limit", "1.0"};
}
...
void AudioSettings::refresh() {
  if (executable_.isEmpty())
    return;
  outputProbe_.run(executable_, {"get-volume", "@DEFAULT_AUDIO_SINK@"},
                   [this](bool ok, const QByteArray &text) {
                     output_ = ok ? parseAudioVolume(text)
                                  : QJsonObject{{"available", false}};
                   });
  inputProbe_.run(executable_, {"get-volume", "@DEFAULT_AUDIO_SOURCE@"},
                  [this](bool ok, const QByteArray &text) {
...
QJsonObject AudioSettings::snapshot() const {
  return {{"installed", !executable_.isEmpty()},
          {"output", output_},
          {"input", input_},
          {"outputDevices", outputDevices_},
          {"busy", action_.busy()},
          {"error", error_}};
}
bool AudioSettings::apply(const QJsonObject &change, QString *error) {
  const auto arguments = audioCommand(change);
  if (arguments.isEmpty() || executable_.isEmpty() || action_.busy()) {
    if (error)
      *error = "Invalid audio setting, unavailable WirePlumber, or another "
               "audio change is pending.";
    return false;
  }
  error_.clear();
  return action_.run(executable_, arguments,
                     [this](bool ok, const QByteArray &) {
                       error_ = ok ? ""
                                   : "Audio change failed. Check the selected "
                                     "device and PipeWire service.";
                       refresh();
                     });
}
} // namespace LunaDash
