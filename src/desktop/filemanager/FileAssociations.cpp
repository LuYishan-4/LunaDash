...
bool validId(const QString &id) {
  if (id.size() > 255 || !id.endsWith(".desktop"))
    return false;
  for (const auto ch : id)
    if (ch.isSpace() || ch.unicode() < 32 || ch == '/' || ch == '\\')
      return false;
  return true;
}
...
bool usable(GAppInfo *app) {
  ...
  const QString executable =
      QFileInfo(QString::fromUtf8(g_app_info_get_executable(app))).fileName();
  ...
  if (executable == "lunadash-desktop" || executable == "ludash-desktop" ||
      ...
      executable == "xdg-open" || executable == "gio")
    return false;
  ...
  const bool busActivation = G_IS_DESKTOP_APP_INFO(app) &&
                             g_desktop_app_info_get_boolean(
                                 G_DESKTOP_APP_INFO(app), "DBusActivatable");
  return validId(QString::fromUtf8(g_app_info_get_id(app))) &&
         (g_app_info_supports_files(app) || g_app_info_supports_uris(app) ||
          busActivation);
}
...
bool validDocument(const QJsonObject &document) {
  const QSet<QString> keys{"version", "initialized", "askOnFirstOpen",
                           "associations"};
  ...
  if (document.value("version").toDouble() != 1 ||
      ...
      !document.value("associations").isObject())
    return false;
  ...
  for (auto it = rules.begin(); it != rules.end(); ++it) {
    ...
    if (!FileAssociations::validKey(it.key()) || !it.value().isObject() ||
        ...
        !validMime(rule.value("mimeType").toString()))
      return false;
  }
  return true;
}
...
FileAssociations::FileAssociations(QString path)
    : path_(path.isEmpty() ? QStandardPaths::writableLocation(
                                 QStandardPaths::GenericConfigLocation) +
                                 "/LunaDash/file-associations.json"
                           : std::move(path)) {}
...
QJsonObject FileAssociations::read(QString *error) const {
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    fail(error, translate("Cannot read file associations. The existing "
  ...
  if (parseError.error != QJsonParseError::NoError || !document.isObject() ||
    ...
    fail(error, translate("Invalid file association configuration. The "
...
QString FileAssociations::keyForExtension(const QString &input) {
  ...
  if (extension.isEmpty() || extension.size() > 64 ||
...
bool FileAssociations::validKey(const QString &key) {
  if (key.startsWith("ext:"))
    return keyForExtension(key.mid(4)) == key;
...
QString FileAssociations::keyForFile(const QString &path) const {
  ...
  if (suffix.isEmpty())
    suffix = info.suffix();
  if (name.startsWith('.') && name.count('.') == 1)
    suffix.clear();
  const auto key = keyForExtension(suffix);
  return key.isEmpty() ? "mime:" + mimeTypeForFile(path) : key;
}
QString FileAssociations::applicationForFile(const QString &path,
                                             QString *error) const {
  return read(error)
      .value("associations")
      .toObject()
      .value(keyForFile(path))
      .toObject()
      .value("desktopId")
      .toString();
}
bool FileAssociations::update(const QString &key, const QJsonObject &value,
  ...
  if (!lock.tryLock(100))
    return fail(error, translate("File associations are being edited in "
                                 "another window. Try again."));
...
bool FileAssociations::setPreferences(bool initialized, bool askOnFirstOpen,
                                      QString *error) const {
  return update(
      {}, {{"initialized", initialized}, {"askOnFirstOpen", askOnFirstOpen}},
      true, error);
}
bool FileAssociations::setRule(const QString &key, const QString &desktopId,
  ...
  if (!fileApplicationAvailable(desktopId))
    return fail(error,
                translate("The selected application is no longer installed."));
  return update(key, {{"desktopId", desktopId}, {"mimeType", mimeType}}, false,
                error);
}
...
QString systemFileApplication(const QString &mimeType) {
  auto *app =
      g_app_info_get_default_for_type(mimeType.toUtf8().constData(), false);
  const QString result =
      usable(app) ? QString::fromUtf8(g_app_info_get_id(app)) : QString{};
  if (app)
    g_object_unref(app);
  return result;
}
QList<FileApplication> fileApplications(const QString &mimeType) {
  ...
  for (auto *item = matches; item; item = item->next)
    recommended.insert(
        QString::fromUtf8(g_app_info_get_id(G_APP_INFO(item->data))));
  ...
  for (auto *item = all; item; item = item->next) {
    ...
    if (seen.contains(id) || (!g_app_info_should_show(app) &&
                              !recommended.contains(id) && id != defaultId))
      continue;
    seen.insert(id);
    result.append({id, QString::fromUtf8(g_app_info_get_display_name(app)),
                   iconName(g_app_info_get_icon(app)), recommended.contains(id),
                   id == defaultId});
  }
...
bool setSystemFileApplication(const QString &desktopId, const QString &mimeType,
  ...
  const bool result = g_app_info_set_as_default_for_type(
      G_APP_INFO(app), mimeType.toUtf8().constData(), &nativeError);
  if (!result)
    fail(error, translate("Could not change the system default: %1")
                    .arg(nativeError ? QString::fromUtf8(nativeError->message)
                                     : QString{}));
  g_clear_error(&nativeError);
  g_object_unref(app);
  return result;
}
bool launchFilesWithApplication(const QString &desktopId,
                                const QStringList &files, QString *error) {
  if (error)
    error->clear();
  ...
  for (const auto &path : files)
    if (!QFileInfo(path).isFile())
      return fail(error, translate("Source no longer exists: %1").arg(path));
  const auto resolved =
      desktopId == "__system__"
          ? systemFileApplication(
                FileAssociations::mimeTypeForFile(files.first()))
          : desktopId;
  ...
  for (const auto &path : files)
    list = g_list_prepend(
        list,
        g_file_new_for_path(
            QFile::encodeName(QFileInfo(path).absoluteFilePath()).constData()));
  list = g_list_reverse(list);
  GError *nativeError = nullptr;
  const bool result =
      g_app_info_launch(G_APP_INFO(app), list, nullptr, &nativeError);
  if (!result)
    fail(error, translate("Could not open the file: %1")
                    .arg(nativeError ? QString::fromUtf8(nativeError->message)
                                     : QString{}));
  g_clear_error(&nativeError);
