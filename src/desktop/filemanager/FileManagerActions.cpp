#include "desktop/filemanager/FileManagerActions.hpp"
...
QString prettyDate(const QDateTime &value) {
  return value.isValid() ? QLocale().toString(value, QLocale::LongFormat)
                         : QStringLiteral("—");
}
} // namespace

FileManagerActions::FileManagerActions(QWidget *page)
    : QObject(page), page_(page),
      details_(page->findChild<QTreeView *>("fileView")),
  ...
  auto action = [this](const QString &id, const QString &label,
                       ...
                       const char *button = nullptr, bool global = false) {
    ...
    if (global)
      ...
      details_->addAction(item);
      icons_->addAction(item);
    }
    ...
    if (button) {
      ...
      connect(item, &QAction::changed, control,
              [=] { control->setEnabled(item->isEnabled()); });
    }
  };

  action("open", translate("Open"), QKeySequence(Qt::Key_Return),
         [this] { openSelection(); });
  action("openWith", translate("Open with…"), {},
         [this] { openSelection(true); });
  action(
      "setDefault", translate("Set default application for this file type..."),
      {}, [this] {
        const auto paths = selection();
        if (paths.size() == 1)
          chooseFileDefault(page_, paths.first(), [this](const QString &error) {
            status_->setText(
                error.isEmpty() ? translate("File association saved.") : error);
          });
      });
  action("resetDefault",
         translate("Reset default application for this file type"), {}, [this] {
           const auto paths = selection();
           if (paths.size() != 1)
             return;
           QString error;
           FileAssociations().removeRule(
               FileAssociations().keyForFile(paths.first()), &error);
           status_->setText(error.isEmpty()
                                ? translate("File association removed.")
                                : error);
         });
  action(
      "copy", translate("Copy"), QKeySequence::Copy,
      [this] { copySelection(false); }, "fileCopy");
  action(
      "cut", translate("Cut"), QKeySequence::Cut,
      [this] { copySelection(true); }, "fileCut");
  action(
      "paste", translate("Paste"), QKeySequence::Paste,
      [this] { paste(currentPath_); }, "filePaste");
  action("duplicate", translate("Duplicate"), QKeySequence("Ctrl+D"),
         [this] { duplicateSelection(); });
  action("copyTo", translate("Copy to…"), {},
         [this] { copyOrMoveSelection(false); });
  action("moveTo", translate("Move to…"), {},
         [this] { copyOrMoveSelection(true); });
  action(
      "rename", translate("Rename"), QKeySequence(Qt::Key_F2),
      [this] { renameSelection(); }, "fileRename");
  action(
      "trash", translate("Move to Trash"), QKeySequence::Delete,
      [this] { trashSelection(); }, "fileTrash");
  action("delete", translate("Delete permanently"),
         QKeySequence("Shift+Delete"),
         [this] { deleteSelectionPermanently(); });
  action(
      "newFile", translate("New file"), QKeySequence("Ctrl+N"),
      [this] { createEntry(false, currentPath_); }, "fileNewFile");
  action(
      "newFolder", translate("New folder"), QKeySequence("Ctrl+Shift+N"),
      [this] { createEntry(true, currentPath_); }, "fileNewFolder");
  action(
      "terminal", translate("Open terminal here"), QKeySequence("Ctrl+Alt+T"),
      [this] { openTerminal(currentPath_); }, "fileTerminal");
  action("properties", translate("Properties"), QKeySequence("Alt+Return"),
         [this] { properties(); });
  action("selectAll", translate("Select all"), QKeySequence::SelectAll,
         [this] { activeView()->selectAll(); });
  action("copyPath", translate("Copy paths"), QKeySequence("Ctrl+Shift+C"),
         [this] {
  ...
  action(
      ...
      nullptr, true);
  actions_.value("refresh")->setShortcuts(
      {QKeySequence("F5"), QKeySequence("Ctrl+R")});
  action("hidden", translate("Show hidden files"), QKeySequence("Ctrl+H"),
         [this] { hidden_->click(); });
  action(
      "location", translate("Location"), QKeySequence("Ctrl+L"),
      [this] {
        location_->setFocus();
        location_->selectAll();
      },
      nullptr, true);
  action(
      "back", translate("Back"), QKeySequence("Alt+Left"),
      [this] {
        if (historyIndex_ > 0)
      ...
      "fileBack", true);
  action(
      "forward", translate("Forward"), QKeySequence("Alt+Right"),
      [this] {
      ...
      "fileForward", true);
  action(
      "up", translate("Up"), QKeySequence("Alt+Up"),
      [this] {
  ...
  connect(page_->findChild<QPushButton *>("fileAssociations"),
          &QPushButton::clicked, this,
          [this] { showFileAssociationSettings(page_); });
  connect(location_, &QLineEdit::returnPressed, this,
          [this] { navigate(location_->text()); });
  connect(places_, &QListWidget::itemClicked, this,
  ...
  connect(
      places_, &QListWidget::customContextMenuRequested, this,
      [this](const QPoint &position) {
        const bool keyboard = position.x() < 0 || position.y() < 0;
        auto *item =
            keyboard ? places_->currentItem() : places_->itemAt(position);
        if (!item)
          return;
        const auto path = item->data(Qt::UserRole).toString();
        if (path.isEmpty())
        ...
        menu->addAction(translate("Open"), this,
                        [this, path] { navigate(path); });
        menu->addAction(translate("Open in a new window"), this, [this, path] {
          if (!QProcess::startDetached(
                  QCoreApplication::applicationFilePath(),
                  {"--app", "files", "--builtin", "--path", path}))
            status_->setText(translate("Could not open a new Files window."));
        });
        menu->addAction(translate("Open terminal here"), this,
                        [this, path] { openTerminal(path); });
        const bool pinned = isPinned(path);
        menu->addAction(
            pinned ? translate("Unpin folder") : translate("Pin folder"), this,
            [this, path, pinned] { setPinned(path, !pinned); });
        menu->addSeparator();
        menu->addAction(translate("Copy paths"), this,
        ...
        menu->addAction(translate("Properties"), this, [this, path] {
          ...
          if (index.isValid())
            details_->selectionModel()->select(
                index, QItemSelectionModel::Select | QItemSelectionModel::Rows);
          properties();
        });
        menu->popup(places_->viewport()->mapToGlobal(
            keyboard ? places_->visualItemRect(item).center() : position));
      });
  ...
  connect(search_, &QLineEdit::textChanged, this, [this](const QString &text) {
    model_->setNameFilters(text.isEmpty() ? QStringList{}
                                          : QStringList{"*" + text + "*"});
  });
  connect(details_->header(), &QHeaderView::sortIndicatorChanged, this,
  ...
  installModel();
  for (auto *view : {static_cast<QAbstractItemView *>(details_),
    ...
    connect(
        view, &QAbstractItemView::clicked, this,
        [this](const QModelIndex &index) {
          if (!index.isValid() || busy_)
            return;
          ...
          if (info.isDir())
            ...
            openAssociatedFiles(page_, {info.absoluteFilePath()}, false,
...
QAbstractItemView *FileManagerActions::activeView() const {
  return mode_->isChecked() ? static_cast<QAbstractItemView *>(icons_)
                            : static_cast<QAbstractItemView *>(details_);
}

QStringList FileManagerActions::selection() const {
  QStringList paths;
  if (!activeView()->selectionModel())
    return paths;
  for (const auto &index : activeView()->selectionModel()->selectedIndexes())
    if (index.column() == 0)
      paths << model_->filePath(index);
...
void FileManagerActions::navigate(const QString &path, bool remember) {
  ...
  if (remember &&
      (historyIndex_ < 0 || history_.value(historyIndex_) != currentPath_)) {
  ...
  folderTitle_->setText(currentPath_ == QDir::homePath() ? translate("Home")
                        : info.fileName().isEmpty() ? translate("File system")
                                                    : info.fileName());
  for (int row = 0; row < places_->count(); ++row)
    places_->item(row)->setSelected(
        places_->item(row)->data(Qt::UserRole).toString() == currentPath_);
  if (!busy_)
    status_->setText(
...
void FileManagerActions::setPinned(const QString &path, bool pinned) {
  ...
  if (pinned && !folders.contains(absolute))
    folders << absolute;
  if (!pinned)
    folders.removeAll(absolute);
  QSettings().setValue("files/pinnedFolders", folders);
  refreshPlaces();
  status_->setText(pinned ? translate("Folder pinned.")
                          : translate("Folder unpinned."));
}

void FileManagerActions::refreshPlaces() {
  places_->clear();
  QSet<QString> visiblePaths;
  auto add = [this, &visiblePaths](const QString &label, const QString &path,
                                   FileIcon icon) {
    if (path.isEmpty())
      return;
    const auto absolute = QFileInfo(path).absoluteFilePath();
    if (visiblePaths.contains(absolute))
      return;
  ...
  for (const auto &path : pinnedFolders())
    add(QStringLiteral("★ ") + QFileInfo(path).fileName(), path,
        FileIcon::Folder);
  add(translate("Desktop"),
      QStandardPaths::writableLocation(QStandardPaths::DesktopLocation),
      FileIcon::Desktop);
  add(translate("Documents"),
      QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation),
      FileIcon::File);
  add(translate("Downloads"),
      QStandardPaths::writableLocation(QStandardPaths::DownloadLocation),
      FileIcon::Download);
  add(translate("Pictures"),
      QStandardPaths::writableLocation(QStandardPaths::PicturesLocation),
      FileIcon::Picture);
  add(translate("Music"),
      QStandardPaths::writableLocation(QStandardPaths::MusicLocation),
      FileIcon::Music);
  add(translate("Videos"),
      QStandardPaths::writableLocation(QStandardPaths::MoviesLocation),
      FileIcon::Video);
  add(translate("File system"), "/", FileIcon::Drive);
  for (const auto &volume : QStorageInfo::mountedVolumes())
    if (volume.isValid() && volume.isReady() &&
        (volume.rootPath().startsWith("/run/media/") ||
         volume.rootPath().startsWith("/media/") ||
         volume.rootPath().startsWith("/mnt/")))
      add(volume.displayName(), volume.rootPath(), FileIcon::Drive);
}

void FileManagerActions::updateActions() {
  const auto paths = selection();
  for (const auto &id : {"open", "copy", "cut", "trash", "duplicate", "copyTo",
                         "moveTo", "delete"})
    actions_.value(id)->setEnabled(!busy_ && !paths.isEmpty());
  const bool files =
      !paths.isEmpty() &&
      std::all_of(paths.begin(), paths.end(),
                  [](const auto &path) { return QFileInfo(path).isFile(); });
  actions_.value("openWith")->setEnabled(!busy_ && files);
  actions_.value("setDefault")
      ->setEnabled(!busy_ && files && paths.size() == 1);
  actions_.value("resetDefault")
      ->setEnabled(
          !busy_ && files && paths.size() == 1 &&
          !FileAssociations().applicationForFile(paths.first()).isEmpty());
  actions_.value("rename")->setEnabled(!busy_ && paths.size() == 1);
  actions_.value("paste")->setEnabled(!busy_ && !clipboardFiles().isEmpty() &&
                                      QFileInfo(currentPath_).isWritable());
  for (const auto &id : {"newFile", "newFolder"})
    actions_.value(id)->setEnabled(!busy_ &&
                                   QFileInfo(currentPath_).isWritable());
...
void FileManagerActions::contextMenu(QAbstractItemView *view,
  ...
  if (index.isValid() &&
      !view->selectionModel()->isSelected(index.siblingAtColumn(0)))
  ...
  if (!paths.isEmpty()) {
    ...
    if (directory) {
      menu->addAction(
          translate("Open in a new window"), this, [this, destination] {
            if (!QProcess::startDetached(
                    QCoreApplication::applicationFilePath(),
                    {"--app", "files", "--builtin", "--path", destination}))
              status_->setText(translate("Could not open a new Files window."));
          });
      ...
      menu->addAction(translate("Open terminal here"), this,
                      [this, destination] { openTerminal(destination); });
    }
    menu->addSeparator();
  ...
  auto *pasteAction = menu->addAction(
      directory ? translate("Paste into this folder") : translate("Paste"),
      this, [this, destination] { paste(destination); });
  pasteAction->setEnabled(!busy_ && !clipboardFiles().isEmpty() &&
                          QFileInfo(destination).isWritable());
  if (paths.isEmpty() || directory) {
    auto *create = menu->addMenu(translate("Create"));
    auto *newFile =
        create->addAction(translate("New file"), this, [this, destination] {
          createEntry(false, destination);
        });
    auto *newFolder =
        create->addAction(translate("New folder"), this, [this, destination] {
          createEntry(true, destination);
        });
  ...
  const QStringList labels{translate("Name"), translate("Size"),
                           translate("Type"), translate("Modified")};
  for (int column = 0; column < labels.size(); ++column) {
    auto *item = sorting->addAction(
        labels.at(column), this, [this, column] { sort(column, ascending_); });
    item->setCheckable(true);
    item->setChecked(sortColumn_ == column);
  }
  sorting->addSeparator();
  auto *descending =
      sorting->addAction(translate("Descending order"), this,
                         [this] { sort(sortColumn_, !ascending_); });
  descending->setCheckable(true);
  descending->setChecked(!ascending_);
  auto *hiddenAction = viewMenu->addAction(translate("Show hidden files"),
                                           hidden_, &QPushButton::click);
  hiddenAction->setCheckable(true);
  hiddenAction->setChecked(hidden_->isChecked());
  viewMenu->addAction(actions_.value("refresh"));
  menu->addAction(translate("File associations"), this,
                  [this] { showFileAssociationSettings(page_); });
  const auto point = keyboard
                         ? (index.isValid() ? view->visualRect(index).center()
                                            : QPoint(16, 16))
                         : position;
...
void FileManagerActions::copySelection(bool cut) {
  ...
  status_->setText(
      cut ? translate("Selection cut. Choose a folder and Paste.")
...
void FileManagerActions::paste(const QString &destination) {
  ...
  if (!paths.isEmpty())
    run(cut ? "move" : "copy", paths, destination, cut);
}
...
void FileManagerActions::run(const QString &operation, const QStringList &paths,
  ...
  connect(watcher, &QFutureWatcher<QString>::finished, this, [=, this] {
    ...
    status_->setText(error.isEmpty() ? translate("Operation completed.")
                                     : error);
    updateActions();
    watcher->deleteLater();
  });
...
void FileManagerActions::createEntry(bool folder, const QString &destination) {
  ...
  dialog->setWindowTitle(folder ? translate("New folder")
                                : translate("New file"));
  dialog->setLabelText(folder ? translate("Folder name")
                              : translate("File name"));
  connect(
      dialog, &QInputDialog::textValueSelected, this,
      [this, destination, folder](const QString &name) {
        if (busy_)
          return;
        bool success = false;
        if (validFileName(name)) {
          if (folder)
            success = QDir(destination).mkdir(name);
          else {
        ...
        status_->setText(
            success
                ? translate("Operation completed.")
                : translate(
                      "Could not create the item. Check its name and "
                      "permissions; existing files are never overwritten."));
      });
  dialog->open();
}
...
void FileManagerActions::renameSelection() {
  ...
  connect(
      dialog, &QInputDialog::textValueSelected, this,
      [this, info](const QString &name) {
        if (busy_ || name == info.fileName())
          return;
        if (!validFileName(name) ||
            QFileInfo::exists(info.dir().filePath(name)) ||
            QFileInfo(info.dir().filePath(name)).isSymLink() ||
            !QDir().rename(info.absoluteFilePath(), info.dir().filePath(name)))
          status_->setText(
              translate("Could not rename item. Nothing was overwritten."));
      });
  dialog->open();
}
...
void FileManagerActions::trashSelection() {
  const auto paths = selection();
  if (busy_ || paths.isEmpty())
    return;
  auto *confirm = new QMessageBox(
      QMessageBox::Question, translate("Move to Trash"),
...
void FileManagerActions::deleteSelectionPermanently() {
  ...
  if (busy_ || paths.isEmpty())
    return;
  auto *confirm = new QMessageBox(
      QMessageBox::Warning, translate("Delete permanently"),
      translate("Permanently delete %1 selected items? This cannot be undone.")
          .arg(paths.size()),
      QMessageBox::Yes | QMessageBox::Cancel, page_);
  configureDialog(confirm);
  confirm->setTextFormat(Qt::PlainText);
...
void FileManagerActions::copyOrMoveSelection(bool move) {
  ...
  auto *dialog = new QFileDialog(
      page_, move ? translate("Move to…") : translate("Copy to…"),
      currentPath_);
  configureDialog(dialog);
  ...
  dialog->setLabelText(QFileDialog::Accept,
                       move ? translate("Move here") : translate("Copy here"));
  connect(dialog, &QFileDialog::fileSelected, this,
          [this, paths, move](const QString &directory) {
            if (!directory.isEmpty())
              run(move ? "move" : "copy", paths, directory);
          });
  dialog->open();
}
...
void FileManagerActions::openTerminal(const QString &directory) {
  ...
  if (!error.isEmpty() || command.isEmpty()) {
    status_->setText(error.isEmpty()
                         ? translate("No default terminal is configured.")
                         : error);
    return;
  }
  QProcess process;
  ...
  if (!process.startDetached())
    status_->setText(translate("Could not start the terminal: %1")
...
void FileManagerActions::properties() {
  ...
  for (const auto &path : paths) {
    ...
    QStringList lines{
        translate("Name") + ": " +
            (info.fileName().isEmpty() ? info.absoluteFilePath()
                                       : info.fileName()),
        translate("Location") + ": " + path,
        translate("Type") + ": " +
            (info.isDir() ? translate("Folder")
                          : FileAssociations::mimeTypeForFile(path)),
        translate("Size") + ": " +
            (info.isDir()
                 ? translate("Folder contents are not included in this size.")
        ...
        translate("Readable") + ": " +
            (info.isReadable() ? translate("Yes") : translate("No")),
        translate("Writable") + ": " +
            (info.isWritable() ? translate("Yes") : translate("No")),
        translate("Executable") + ": " +
            (info.isExecutable() ? translate("Yes") : translate("No"))};
    if (info.isSymLink())
      lines << translate("Symbolic link target") + ": " + info.symLinkTarget();
    sections << lines.join('\n');
  }
...
void FileManagerActions::sort(int column, bool ascending) {
  ...
  details_->sortByColumn(sortColumn_,
                         ascending ? Qt::AscendingOrder : Qt::DescendingOrder);
  model_->sort(sortColumn_,
               ascending ? Qt::AscendingOrder : Qt::DescendingOrder);
}
...
void FileManagerActions::updatePreview(const QString &path) {
  ...
  const QString mime = info.isDir() ? QStringLiteral("inode/directory")
                                    : FileAssociations::mimeTypeForFile(target);
  QStringList meta{
  ...
  if (info.isDir()) {
    ...
    previewHint_->setText(isPinned(target)
                              ? translate("Pinned folder")
                              : translate("Right-click to pin this folder."));
    return;
  }
  ...
  if (mime.startsWith("video/")) {
    ...
    if (QFileInfo::exists(cache)) {
      ...
      if (!thumbnail.isNull()) {
        previewImage_->setPixmap(
            thumbnail.scaled(previewImage_->size().expandedTo(QSize(240, 190)),
    ...
    if (!program.isEmpty()) {
      ...
      if (!program.isEmpty())
        arguments = {"-y",        "-ss", "00:00:01", "-i",           target,
                     "-frames:v", "1",   "-vf",      "scale=512:-1", cache};
    }
    ...
    if (!program.isEmpty()) {
      ...
      connect(process,
              ...
              [this, process, cache, target](int code,
                                             QProcess::ExitStatus status) {
                process->deleteLater();
                if (previewPath_ != QFileInfo(target).absoluteFilePath())
                  return;
                if (status == QProcess::NormalExit && code == 0 &&
                    QFileInfo::exists(cache)) {
                  const QPixmap thumbnail(cache);
                  if (!thumbnail.isNull()) {
                    previewImage_->setPixmap(thumbnail.scaled(
                        previewImage_->size().expandedTo(QSize(240, 190)),
                        Qt::KeepAspectRatio, Qt::SmoothTransformation));
                    previewImage_->setText({});
                    previewHint_->setText(translate("Video thumbnail preview"));
                  }
    ...
    previewImage_->setPixmap(fileIcon(FileIcon::Video).pixmap(128, 128));
    previewHint_->setText(
        translate("Install ffmpeg or ffmpegthumbnailer for video previews."));
    return;
  }

  previewImage_->setPixmap(fileIcon(FileIcon::File).pixmap(112, 112));
  previewHint_->setText(
      translate("Hover another file to preview it without opening it."));
}

bool FileManagerActions::eventFilter(QObject *watched, QEvent *event) {
  auto *view = watched == details_->viewport()
                   ? static_cast<QAbstractItemView *>(details_)
               : watched == icons_->viewport()
                   ? static_cast<QAbstractItemView *>(icons_)
                   : nullptr;
  if (!view)
  ...
  const auto destination = index.isValid() && model_->isDir(index)
  ...
  if (event->type() == QEvent::Drop)
    run(move ? "move" : "copy", paths, destination);
  return true;
}
} // namespace LunaDash
