...
QList<ApplicationEntry> discoverApplications() {
  ...
  for (const auto &root : roots) {
    QDirIterator iterator(root, {"*.desktop"}, QDir::Files,
    ...
    while (iterator.hasNext()) {
      ...
      if (file.value("Type").toString() != "Application" ||
          file.value("Hidden").toBool() || file.value("NoDisplay").toBool() ||
          file.value("Terminal").toBool())
        continue;
      // Qt's argument tokenizer does not implement all Desktop Entry escaping
      // rules. Reject field codes requiring files/URLs rather than inventing an
      // input.
      const auto command = file.value("Exec").toString();
      auto args = QProcess::splitCommand(command);
      if (args.isEmpty())
        continue;
      const auto name = (selectedLanguage() == "zh_TW"
                             ? file.value("Name[zh_TW]", file.value("Name"))
                             : file.value("Name"))
                            .toString();
      ...
      if (unsupported || expanded.isEmpty())
        continue;
      auto program = expanded.takeFirst();
      if (QStandardPaths::findExecutable(program).isEmpty())
        continue;
      entries.append(
          {name, icon, program, expanded, file.value("Path").toString()});
    }
  }
