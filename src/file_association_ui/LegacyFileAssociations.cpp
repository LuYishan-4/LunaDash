...
bool migrateLegacyFileAssociations(QString* error) {
    ...
    QSettings settings;
    if (settings.value("files/associationMigrationVersion", 0).toInt() >= 1) return true;
    ...
    for (const auto& oldKey : settings.childKeys()) {
        ...
        if (oldKey.startsWith("ext_")) {
            ...
            mime = QMimeDatabase().mimeTypeForFile("example." + oldKey.mid(4), QMimeDatabase::MatchExtension).name();
        } else if (oldKey.startsWith("mime_")) {
            mime = oldKey.mid(5);
            const auto separator = mime.indexOf('_');
            if (separator >= 0) mime[separator] = '/';
            key = "mime:" + mime;
        }
        if (!FileAssociations::validKey(key) || existing.contains(key)) continue;
        const auto oldValue = settings.value(oldKey).toString();
        const auto id = oldValue == "__system__" ? oldValue : QFileInfo(oldValue).fileName();
        if (!fileApplicationAvailable(id)) { ++unavailable; continue; }
        QString failure;
    ...
    if (settings.status() != QSettings::NoError) {
        if (error) *error = translate("Could not save file associations.");
        return false;
    }
    if (unavailable && error)
        *error = translate("Some previous file associations refer to unavailable applications. The old settings were kept; choose a new application when opening those files.");
    return true;
}
} // namespace LuDash
