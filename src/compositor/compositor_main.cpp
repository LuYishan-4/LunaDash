...
int main(int argc, char **argv) {
  ...
  parser.addOption({"fullscreen", "Prefer the host output size in nested mode."});
  ...
  if (parser.isSet("profile")) {
    ...
    QObject::connect(dispatcher, &QAbstractEventDispatcher::aboutToBlock, &app,
                     [&] {
                       ...
                       if (elapsed > 25)
                         qWarning() << "LunaDash event-loop stall:" << elapsed
                                    << "ms";
                     });
  }

  LuDash::WaylandCompositor compositor(
      parser.value("socket").toUtf8(), parser.isSet("fullscreen"),
