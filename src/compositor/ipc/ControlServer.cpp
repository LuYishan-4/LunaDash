...
ControlServer::ControlServer(
  ...
  connect(server_, &QLocalServer::newConnection, this, [this, handler] {
    while (auto *socket = server_->nextPendingConnection()) {
      ...
      QTimer::singleShot(3000, socket,
                         [socket] { socket->disconnectFromServer(); });
      connect(socket, &QLocalSocket::disconnected, socket,
              &QObject::deleteLater);
      connect(
          socket, &QLocalSocket::readyRead, socket, [socket, buffer, handler] {
            ...
            const auto input = QJsonDocument::fromJson(
                buffer->left(buffer->indexOf('\n')), &error);
            const auto response =
                error.error == QJsonParseError::NoError && input.isObject()
                    ? handler(input.object())
                    : QJsonObject{{"error", "Invalid request"}};
            socket->write(
