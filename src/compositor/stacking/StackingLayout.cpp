...
StackingLayout::StackingLayout() : d(std::make_unique<Impl>()) {}
StackingLayout::~StackingLayout() = default;
WindowLayoutMode StackingLayout::mode() const noexcept {
  return WindowLayoutMode::Stacking;
}
void StackingLayout::setGap(int) {}
bool StackingLayout::insert(LayoutWorkspaceId workspace, LayoutWindowId window,
                            QSize preferred) {
...
bool StackingLayout::remove(LayoutWindowId window) {
...
bool StackingLayout::setMinimized(LayoutWindowId window, bool minimized) {
  if (!d->entries.contains(window))
    return false;
...
bool StackingLayout::setMaximized(LayoutWindowId window, bool maximized) {
  if (!d->entries.contains(window))
    return false;
...
bool StackingLayout::moveToWorkspace(LayoutWindowId window,
...
bool StackingLayout::focus(LayoutWindowId window) {
...
bool StackingLayout::focusRight(LayoutWorkspaceId workspace) {
  for (auto id : d->order)
    if (d->entries[id].workspace == workspace && !d->entries[id].minimized)
      return focus(id);
  d->focused.remove(workspace);
...
bool StackingLayout::focusLeft(LayoutWorkspaceId workspace) {
...
bool StackingLayout::focusUp(LayoutWorkspaceId workspace) {
  return focusLeft(workspace);
}
bool StackingLayout::focusDown(LayoutWorkspaceId workspace) {
  return focusRight(workspace);
}
bool StackingLayout::groupWith(LayoutWindowId, LayoutWindowId) { return false; }
bool StackingLayout::expel(LayoutWindowId) { return false; }
bool StackingLayout::insertBeside(LayoutWindowId, LayoutWindowId, bool) {
  return false;
}
bool StackingLayout::swapWindows(LayoutWindowId window, LayoutWindowId target) {
...
bool StackingLayout::resize(LayoutWindowId window, int width) {
...
bool StackingLayout::resizeHeight(LayoutWindowId window, int height) {
  if (!d->entries.contains(window) || !d->entries[window].geometry.isValid())
    return false;
  auto &entry = d->entries[window];
...
bool StackingLayout::moveSingle(LayoutWindowId window, QPoint delta,
                                QRect area) {
  if (!d->entries.contains(window) || !d->entries[window].geometry.isValid() ||
      !area.isValid())
    return false;
...
bool StackingLayout::reorder(LayoutWindowId window, int direction) {
  if (!d->entries.contains(window))
    return false;
  if (direction >= 0)
    return focus(window);
...
bool StackingLayout::center(LayoutWindowId window, QRect area) {
  if (!d->entries.contains(window) || !area.isValid())
    return false;
  d->entries[window].geometry.moveCenter(area.center());
  return true;
}
QList<WindowPlacement> StackingLayout::layout(LayoutWorkspaceId workspace,
                                              QRect area) {
  if (!area.isValid())
    return {};
...
QList<WindowPlacement> StackingLayout::presentation(LayoutWorkspaceId workspace,
                                                    QRect area) {
...
StackingLayout::snapshot(LayoutWorkspaceId workspace) const {
