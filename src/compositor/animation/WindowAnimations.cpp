#include "compositor/animation/WindowAnimations.hpp"
...
void WindowAnimations::setDuration(int milliseconds) {
  ...
  if (!duration_) {
    ...
    for (auto *item : items) {
      ...
      if (auto *group = active_.value(item))
        group->setCurrentTime(group->duration());
    }
  }
}
int WindowAnimations::activeCount() const {
...
void WindowAnimations::hide(QQuickItem *item,
                            const std::function<void()> &finished) {
  animate(item, false, finished);
}
void WindowAnimations::animate(QQuickItem *item, bool showing,
                               const std::function<void()> &finished) {
  const bool alreadyAnimating = active_.contains(item);
  cancel(item);
  if (!duration_) {
    item->setOpacity(showing ? .999 : 0);
    item->setScale(1);
    item->setVisible(showing);
    if (finished)
      finished();
    return;
  }
  if (showing && !alreadyAnimating) {
    item->setOpacity(0);
    item->setScale(.94);
  }
  ...
  for (const auto &property : {QByteArray("opacity"), QByteArray("scale")}) {
    ...
    animation->setEndValue(property == "opacity" ? (showing ? .999 : 0.0)
                                                 : (showing ? 1.0 : .94));
    animation->setDuration(duration_);
    animation->setEasingCurve(showing ? QEasingCurve::OutCubic
                                      : QEasingCurve::InCubic);
    group->addAnimation(animation);
  }
  active_.insert(item, group);
  connect(group, &QParallelAnimationGroup::finished, this,
          [this, item, group, showing, finished] {
            if (active_.value(item) != group)
              return;
            active_.remove(item);
            group->deleteLater();
            if (!showing)
