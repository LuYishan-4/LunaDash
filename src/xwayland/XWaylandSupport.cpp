...
bool writeAuthority(const QString &path, const QString &display) {
  ...
  stream << quint16(
      65535); // FamilyWild: local-only transport still requires the cookie.
...
XWaylandSupport::XWaylandSupport(QObject *parent) : QObject(parent) {
  ...
  connect(&server_, &QProcess::finished, this,
          [this](int code, QProcess::ExitStatus status) {
            ...
            // Reap only the process group created for this compatibility
            // instance.
            if (groupId_ > 0)
...
bool XWaylandSupport::start(const QProcessEnvironment &environment,
  ...
  // Force Xwayland's shared-memory backend. The compositor advertises wl_drm
  // only through the EGL bind path, which is unavailable in software/nested
  // sessions; glamor then fails with "invalid format" and drops the display,
  // leaving X11 applications with no usable X server. SHM keeps GLX available
  // through llvmpipe and matches the software shell renderer.
...
QJsonObject XWaylandSupport::snapshot() const {
  return {{"available",
          ...
          {"mode", "rootful"},
          {"running", !stopping_ && server_.state() == QProcess::Running},
          {"display", display_},
          {"authority", authority_},
          {"error", error_}};
}
void XWaylandSupport::applyEnvironment(QProcessEnvironment &environment) const {
  if (!display_.isEmpty() && error_.isEmpty() && !stopping_) {
...
bool XWaylandSupport::launch(const QStringList &command, QString *error) {
  ...
  // Xwayland runs with SHM, so hardware GLX/EGL is unavailable; force software
  // GL so Electron/Chromium apps such as Discord render instead of dying.
