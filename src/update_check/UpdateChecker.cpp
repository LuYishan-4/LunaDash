...
UpdateChecker::UpdateChecker(QObject *parent):QObject(parent),network_(new QNetworkAccessManager(this)),timeout_(new QTimer(this)){channel_=QSettings().value("updates/channel","stable").toString();if(channel_!="stable"&&channel_!="dev")channel_="stable";timeout_->setSingleShot(true);timeout_->setInterval(8000);connect(timeout_,&QTimer::timeout,this,[this]{if(reply_)reply_->abort();finishWithError("The update check timed out.");});}
