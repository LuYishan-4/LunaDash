...
QString currentCommit(){QString commit=QStringLiteral(LUNADASH_GIT_COMMIT);if(!commit.isEmpty()&&commit!="unknown")return commit;QProcess git;git.start("git",{"rev-parse","--verify","HEAD"});if(git.waitForFinished(800)&&git.exitCode()==0){commit=QString::fromUtf8(git.readAllStandardOutput()).trimmed();if(commit.size()>=12)return commit;}return QStringLiteral("unknown");}
}
