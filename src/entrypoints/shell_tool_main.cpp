...
QJsonObject mediaSnapshot(const QString &service) {
  ...
  QDBusInterface player(service, kMprisPath, "org.mpris.MediaPlayer2.Player", bus);
...
int main(int argc, char **argv) {
  ...
  if (arguments.size() < 2)
    return printJson({{"error", "Expected media-status, media-action, or language."}});
  ...
  if (arguments[1] == "language") {
    ...
    if (!QRegularExpression("^[a-z]{2,3}(?:_[A-Z]{2})?$").match(locale).hasMatch())
