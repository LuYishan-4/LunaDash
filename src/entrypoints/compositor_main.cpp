...
int main(int argc, char **argv) {
  ...
  parser.addOption({"exit-after", "Exit after this many milliseconds (test mode).", "ms"});
  parser.process(app);
  ...
  if (parser.isSet("demo"))
    QTimer::singleShot(900, &app, [&] {
      if (!parser.isSet("no-shell")) compositor.spawn({"--app", "welcome"});
      compositor.spawn({"--app", "files"});
      compositor.spawn({"--app", "monitor"});
    });
  if (parser.isSet("exit-after"))
    QTimer::singleShot(std::max(1000, parser.value("exit-after").toInt()), &app, [&] {
      if (parser.isSet("state")) compositor.saveState(parser.value("state"));
      bool ok = true;
      if (parser.isSet("screenshot")) ok = compositor.saveScreenshot(parser.value("screenshot"));
