...
int main(int argc, char** argv) {
    ...
    parser.addOption({"builtin", "Open the built-in file manager, ignoring the default application."});
    ...
    if (parser.isSet("app")) {
        ...
        if (requested == "terminal" || (requested == "files" && !parser.isSet("builtin"))) {
            QString error; auto command = LuDash::defaultApplicationCommand(requested, &error);
            if (!error.isEmpty()) { qCritical().noquote() << error; return 2; }
            if (!command.isEmpty()) { const auto program = command.takeFirst(); if (requested == "files" && parser.isSet("path")) command << QFileInfo(parser.value("path")).absoluteFilePath(); return QProcess::execute(program, command); }
        }
        ...
        content->setObjectName("applicationContent"); content->setAttribute(Qt::WA_StyledBackground);
        auto* layout = new QVBoxLayout(&window); layout->setContentsMargins(0, 0, 0, 0); layout->addWidget(content);
        if (id == "files" && parser.isSet("path")) { auto* location = content->findChild<QLineEdit*>("fileLocation"); location->setText(QFileInfo(parser.value("path")).absoluteFilePath()); QMetaObject::invokeMethod(location, "returnPressed"); }
        window.setWindowTitle("LuDash · " + id); window.resize(id == "files" ? 1040 : 760, id == "files" ? 680 : 520); window.show();
