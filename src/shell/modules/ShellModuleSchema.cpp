...
QStringList shellModuleIds() {
  return {"panel", "wallpaper", "launcher", "overview",     "settings",
          "setup", "session",   "feedback", "compatibility"};
}
...
QJsonObject defaultModuleDocument() {
  ...
  for (const auto &id : shellModuleIds()) {
    ...
    if (id == "launcher") {
      ...
      config = QJsonObject{
          {"backgroundVisible", false}, {"contrastShells", true},
          {"workspacePills", true},     {"workspaceInactiveWidth", 18},
          {"workspaceActiveWidth", 38}, {"workspacePillHeight", 10},
          {"shellOpacity", 20},         {"workspaceTransition", true},
...
bool integer(const QJsonValue &value, int low, int high) {
  ...
  return value.isDouble() && std::isfinite(n) && std::floor(n) == n &&
         n >= low && n <= high;
}
...
bool validOverviewShortcuts(const QJsonValue &value) {
  if (!value.isArray() || value.toArray().size() > 6)
    return false;
  const QStringList allowed{"files",   "terminal", "settings",
                            "monitor", "network",  "plugins"};
  QSet<QString> seen;
  for (const auto &item : value.toArray()) {
    if (!item.isString() || !allowed.contains(item.toString()) ||
...
bool validateModuleDocument(const QByteArray &text, QJsonObject *normalized,
  ...
  for (auto it = input.begin(); it != input.end(); ++it) {
      ...
      for (auto field = changes.begin(); field != changes.end(); ++field) {
        ...
        if (key == "width")
          valid = integer(value, 0, 0) ||
                  integer(value, id == "settings" ? 800 : 320, 3840);
        else if (key == "height")
          valid = integer(value, 0, 0) ||
                  integer(value,
                          id == "panel"                       ? 24
                          : id == "settings" || id == "setup" ? 480
                                                              : 80,
                          id == "panel" ? 96 : 2160);
        else if (key == "x" || key == "y")
          valid = integer(value, 0, 3840);
        else if (key == "margin" || key == "radius")
          valid = integer(value, 0, 64);
        else if (key == "fontSize")
          valid = integer(value, 10, 28);
        else if (key == "background" || key == "foreground" || key == "accent")
          valid =
              value.isString() && (value.toString() == "inherit" ||
                                   color.match(value.toString()).hasMatch());
        else if (key == "edge")
          valid = value.isString() &&
                  (value.toString() == "top" || value.toString() == "bottom") &&
                  (id == "panel" || value.toString() == "top");
        if (!valid)
          return fail(error, "Invalid style field: " + id + "." + key);
        style[key] = value;
      }
      module["style"] = style;
    }
    ...
    if (source.contains("config")) {
      ...
      if (id != "launcher" && id != "panel" && id != "overview" &&
          !changes.isEmpty())
        return fail(error,
                    "This module does not have module-specific config fields.");
      auto config = module.value("config").toObject();
      if (id == "launcher") {
        if (!keysAllowed(changes, {"buttonSize", "logoScale",
                                   "backgroundOpacity", "glow", "orbit"}))
        ...
        for (auto field = changes.begin(); field != changes.end(); ++field) {
          ...
          bool valid = false;
          if (key == "buttonSize")
            valid = integer(value, 28, 64);
          else if (key == "logoScale")
            valid = integer(value, 50, 120);
          else if (key == "backgroundOpacity")
            valid = integer(value, 0, 100);
          else if (key == "glow" || key == "orbit")
            valid = value.isBool();
          if (!valid)
            return fail(error, "Invalid launcher config field: " + key);
          config[key] = value;
        }
      } else if (id == "panel") {
        if (!keysAllowed(changes,
                         {"backgroundVisible", "contrastShells",
                          "workspacePills", "workspaceInactiveWidth",
                          "workspaceActiveWidth", "workspacePillHeight",
                          "shellOpacity", "workspaceTransition",
                          "transitionDuration"}))
        ...
        for (auto field = changes.begin(); field != changes.end(); ++field) {
          ...
          const auto value = field.value();
          bool valid = false;
          if (key == "backgroundVisible" || key == "contrastShells" ||
              key == "workspacePills" || key == "workspaceTransition")
            valid = value.isBool();
          else if (key == "workspaceInactiveWidth")
            valid = integer(value, 8, 48);
          else if (key == "workspaceActiveWidth")
            valid = integer(value, 18, 72);
          else if (key == "workspacePillHeight")
            valid = integer(value, 4, 20);
          else if (key == "shellOpacity")
            valid = integer(value, 0, 60);
          else if (key == "transitionDuration")
            valid = integer(value, 180, 900);
          if (!valid)
            return fail(error, "Invalid panel config field: " + key);
          config[key] = value;
        }
        if (config.value("workspaceActiveWidth").toInt() <
            config.value("workspaceInactiveWidth").toInt())
          return fail(error, "Active workspace pill must be at least as wide "
                             "as an inactive pill.");
      } else if (id == "overview") {
        if (!keysAllowed(changes, {"showMedia", "quickControls",
                                   "calendarImage", "shortcuts"}))
          return fail(error, "Unknown overview config field.");
        for (auto field = changes.begin(); field != changes.end(); ++field) {
          const auto key = field.key();
          const auto value = field.value();
          bool valid = false;
          if (key == "showMedia" || key == "quickControls")
            valid = value.isBool();
          else if (key == "calendarImage") {
            const QString image = value.toString();
            valid =
                value.isString() && image.size() <= 4096 &&
                (image.isEmpty() ||
                 (image.startsWith("file://") &&
                  QRegularExpression("\\.(?:png|jpe?g|webp|gif)$",
                                     QRegularExpression::CaseInsensitiveOption)
                      .match(image)
                      .hasMatch()));
          } else if (key == "shortcuts")
            valid = validOverviewShortcuts(value);
          if (!valid)
            return fail(error, "Invalid overview config field: " + key);
          config[key] = value;
        }
      }
    ...
    if (source.contains("custom")) {
      ...
      if (!keysAllowed(custom, {"enabled", "entry"}) ||
          !custom.value("enabled").isBool() ||
          !custom.value("entry").isString())
        return fail(error,
                    "Custom modules require boolean enabled and string entry.");
      ...
      if ((!path.isEmpty() && !entry.match(path).hasMatch()) ||
        ...
        return fail(
            error,
            "Entry must be a relative module/Main.qml path with no traversal.");
