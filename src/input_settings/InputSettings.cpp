...
public:
  ...
  void stop() {
    notifier_.reset();
    for (auto *device : keyboards_) libinput_device_unref(device);
    keyboards_.clear();
    if (input_) { libinput_unref(input_); input_ = nullptr; }
    if (udev_) { udev_unref(udev_); udev_ = nullptr; }
    dispatchScheduled_ = false;
    ledStateValid_ = false;
  }
  void configure(const QString &layout) {
    if (!context_) context_ = xkb_context_new(XKB_CONTEXT_NO_FLAGS);
    if (!context_) return;
    if (state_) xkb_state_unref(state_);
    if (keymap_) xkb_keymap_unref(keymap_);
    state_ = nullptr; keymap_ = nullptr;
    ...
    if (keymap_) state_ = xkb_state_new(keymap_);
    ensureInput();
    syncLeds(true);
  }
private:
  ...
  void ensureInput() {
    ...
    if (!input_ || libinput_udev_assign_seat(input_, "seat0") != 0) {
      ...
      input_ = nullptr;
      udev_unref(udev_); udev_ = nullptr;
      return;
    }
  ...
  void scheduleDispatch() {
    ...
    QTimer::singleShot(0, this, [this] { dispatchScheduled_ = false; dispatchBatch(); });
  }
  void dispatchBatch() {
    if (!input_ || libinput_dispatch(input_) != 0) return;
    ...
    while (processed < kMaxEventsPerTurn) {
      ...
      if (type == LIBINPUT_EVENT_DEVICE_ADDED) {
        ...
        if (libinput_device_has_capability(device, LIBINPUT_DEVICE_CAP_KEYBOARD) && std::find(keyboards_.begin(), keyboards_.end(), device) == keyboards_.end()) {
          libinput_device_ref(device); keyboards_.push_back(device); ledsDirty = true;
        }
      } else if (type == LIBINPUT_EVENT_DEVICE_REMOVED) {
        auto *device = libinput_event_get_device(event);
        const auto it = std::find(keyboards_.begin(), keyboards_.end(), device);
        if (it != keyboards_.end()) { libinput_device_unref(*it); keyboards_.erase(it); }
        ...
        xkb_state_update_key(state_, key, libinput_event_keyboard_get_key_state(keyboard) == LIBINPUT_KEY_STATE_PRESSED ? XKB_KEY_DOWN : XKB_KEY_UP);
        ledsDirty = true;
      }
      libinput_event_destroy(event);
    }
    if (ledsDirty) syncLeds(false);
    if (processed == kMaxEventsPerTurn) scheduleDispatch();
  }
  void syncLeds(bool force) {
    ...
    if (!force && ledStateValid_ && leds == lastLeds_) return;
    lastLeds_ = leds; ledStateValid_ = true;
    for (auto *device : keyboards_) libinput_device_led_update(device, leds);
  }
  udev *udev_ = nullptr;
