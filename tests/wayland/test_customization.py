...
with tempfile.TemporaryDirectory(prefix="ludash-customization-") as runtime:
    ...
    def request(method="status", value=""):
        with socket.socket(socket.AF_UNIX) as connection:
            ...
            connection.connect(runtime + "/ludash-custom-control")
            connection.sendall(
    ...
    with open(build / "customization.log", "w+") as log:
        ...
        try:
            ...
            print(
                f"Customization desktop ready in {time.monotonic() - started:.1f}s.",
                flush=True,
            )
            ...
            document = {
                ...
                "modules": {
                    "panel": {
                        ...
                        "custom": {"enabled": True, "entry": "panel/Main.qml"},
                    }
                },
            }
            ...
            request("module-code-trust", "true")
            time.sleep(1.5)
            ...
            # Open settings over the control socket: the trusted panel template
            # replaces the built-in selector, so its position is not predictable.
            assert "error" not in request("open-settings", "")
            ...
            wait_for(
                lambda s: s["panelExtent"] == 40 and not s["shellModules"]["trusted"]
            )
            # Dismiss settings from the restored built-in panel. With the default
            # 40 px panel, zero margin and a 114 px centred selector the settings
            # button is centred on 763, 20 (qml/panel/TopPanel.qml).
            time.sleep(1)
            subprocess.run(
                ["xdotool", "mousemove", "--window", window, "763", "20", "click", "1"],
                check=True,
            )
            ...
            state = wait_for(
                lambda s: any(
                    c["mapped"] and c["id"] not in known for c in s["clients"]
                )
            )
            client = next(
                c for c in state["clients"] if c["id"] not in known and c["mapped"]
            )
            ...
            known = {c["id"] for c in request()["clients"]}
            result = request("launch-default", "terminal")
            ...
            request(
                "focus",
                next(c for c in terminal["clients"] if c["id"] not in known)["id"],
            )
            time.sleep(0.7)
            subprocess.run(
                [
            ...
            while time.monotonic() < deadline and (
                not marker.exists() or not marker.read_text().strip()
            ):
                time.sleep(0.1)
            ...
            subprocess.run(["xdotool", "key", "Return"], check=True)
            wait_for(lambda s: not s["clients"])
            assert process.wait(timeout=32) == 0, (
