...
Switch {
    ...
    indicator: Rectangle {
        ...
        radius: 11; color: control.checked ? Theme.accent : Theme.track
        border.width: control.activeFocus ? 1 : 0; border.color: Theme.text
        Rectangle { x: control.checked ? parent.width - width - 3 : 3; y: 3; width: 16; height: 16; radius: 8; color: control.checked ? Theme.accentInk : Theme.knob; Behavior on x { NumberAnimation { duration: Math.min(Theme.motion, 160) } } }
    }
