...
ColumnLayout {
    ...
    SettingsComponents.SettingsCard {
        ...
        description:page.selectedChannel==="dev"?shell.tr("Development follows the latest commit on the dev branch."):shell.tr("Stable follows published GitHub Releases from the main branch.")
        RowLayout { Layout.fillWidth:true; Text { text:shell.tr("Channel"); color:Theme.muted } ComboBox { model:[shell.tr("Stable (main / releases)"),shell.tr("Development (dev / commits)")]; currentIndex:page.selectedChannel==="dev"?1:0; onActivated:{ const channel=currentIndex===1?"dev":"stable"; shell.command("appearance",JSON.stringify({updateChannel:channel})); } } Item { Layout.fillWidth:true } ShellButton { text:shell.tr("Check now"); enabled:page.update.status!=="checking"; onClicked:shell.command("check-update","") } }
