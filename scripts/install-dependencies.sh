...
case "$manager" in
    pacman)
        ...
        run "${elevate[@]}" "${pacman_install[@]}" \
            ...
            shared-mime-info fish grim slurp brightnessctl \
    ...
    apt)
        ...
        run "${elevate[@]}" env DEBIAN_FRONTEND=noninteractive apt-get install -y \
            ...
            libqt6opengl6-dev shared-mime-info fish grim slurp brightnessctl \
    ...
    xbps)
        run "${elevate[@]}" xbps-install -Sy \
            ...
            qt6-wayland shared-mime-info fish grim slurp brightnessctl \
            fcitx5 fcitx5-qt fcitx5-configtool
        ;;
    emerge)
        run "${elevate[@]}" emerge --noreplace \
            ...
            gui-apps/grim gui-apps/slurp app-misc/brightnessctl x11-misc/shared-mime-info app-shells/fish \
