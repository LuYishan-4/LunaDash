...
Restart LunaDash after installing the optional packages. The compositor registers `wp_viewporter`, prepares a private display and starts XWayland on the first explicit X11 launch. Native Wayland applications continue to use Wayland. A normal launch does not pin a toolkit backend: the client environment asks for `QT_QPA_PLATFORM=wayland;xcb`, `GDK_BACKEND=wayland,x11` and `SDL_VIDEODRIVER=wayland,x11`, so each toolkit can try Wayland first. Before the compatibility server is actually started, the child environment intentionally omits `DISPLAY`; use an explicit X11 launch for an X11-only client. Discord, Electron-style applications, kitty and the Chromium-based browsers (Chrome, Chromium, Edge, Brave, Vivaldi, Opera) are routed to XWayland explicitly: their native Wayland backends need a compositor that hands out GPU buffers over Wayland, and Chromium reports that its Wayland backend refuses Vulkan on drivers that cannot bind EGL to Wayland. Set `LUNADASH_CHROMIUM_WAYLAND=1` before starting the session to send the Chromium-based browsers to native Wayland with explicit Ozone flags instead. Either route logs an abnormal exit with the program name and exit status, because a browser or editor that refuses to start leaves no window to inspect. Missing compatibility tools do not prevent the native desktop from starting.
