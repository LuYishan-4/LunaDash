...
LunaDah remains a Wayland compositor. Optional XWayland runs a rootful compatibility window that uses LunaDah's normal tiling, focus and effects. X11 applications share the inside of this window; they are not individually tiled. LunaDah does not implement a separate X11 window manager.
...
./build/lunadahctl launch-x11 'application --argument'
...
Compatibility depends on the installed XWayland version and the application's protocol use. Clipboard, drag-and-drop, games, input grabs, HiDPI, menus and Wine/Java applications need broader application-specific testing. The integration test is not evidence that every legacy application works. Multiple outputs, pointer constraints and full portal integration remain incomplete in LunaDah.
