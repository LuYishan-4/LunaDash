...
LuDash remains a Wayland compositor. Optional XWayland runs a rootful compatibility window that uses LuDash's normal tiling, focus and effects. X11 applications share the inside of this window; they are not individually tiled. LuDash does not implement a separate X11 window manager.
...
Restart LuDash after installing the optional packages. The compositor registers `wp_viewporter`, prepares a private display and starts XWayland on the first explicit X11 launch. Native Wayland applications continue to use Wayland. Missing compatibility tools do not prevent the native desktop from starting.
...
./build/ludashctl launch-x11 'application --argument'
...
Compatibility depends on the installed XWayland version and the application's protocol use. Clipboard, drag-and-drop, games, input grabs, HiDPI, menus and Wine/Java applications need broader application-specific testing. The integration test is not evidence that every legacy application works. Multiple outputs, pointer constraints and full portal integration remain incomplete in LuDash.
