...
離開或最小化最後一個 X11 視窗時，會另外更新 XWayland 的啟用狀態。原生 foreign-toplevel 更新統一經過檢查視窗類型的入口，避免把 `XWaylandState` 當成 xdg `ToplevelState`。XWayland 整合測試會反覆切到空工作區再切回，並最小化／還原 X11 client，涵蓋 wlroots 0.20 上實際擷取到的 Spotify 工作區切換崩潰；此問題不需要播放音樂或啟用 Audio Wave 才會觸發。
