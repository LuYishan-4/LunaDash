...
預設視窗排列把第一個視窗保留在右半部，後續視窗分割目前焦點格子並交替分割方向。沿用既有 bounded tiling 模板建立參考配置；後續分割優先保留每格至少 320 × 220 邏輯像素，空間不足時改分割較大的窗格，因此原本的大窗之後也可能分割。設定 > 視窗與工作區可調整分割目標及初始方向，詳見[視窗配置模板](WINDOW_LAYOUT_TEMPLATES.md)。
