...
底部快捷 Dock 預設關閉，可在設定 > 外觀 > 顯示 Dock 手動啟用；既有使用者儲存的 Dock 偏好會保留。啟用後，固定項目來自 `dock` 模組設定，執行中的視窗沿用原有 task 啟用行為；目前工作區有可見應用程式時，自動隱藏會保留 8 px 的喚出區域。

預設視窗排列把第一個視窗保留在右半部，後續視窗分割目前焦點格子並交替分割方向。沿用既有 bounded tiling 模板即可形成參考圖中的右側大窗及左側逐層分割窗格；設定 > 視窗與工作區可調整分割目標及初始方向，詳見[視窗配置模板](WINDOW_LAYOUT_TEMPLATES.md)。
...
設定 > 外觀 > 面板配置可調整置中啟動器、目前視窗標題、CPU／記憶體資訊及精簡工作區清單。關閉置中啟動器後，啟動器回到左側，時鐘回到中央。設定 > Shell 模組提供其餘經驗證的面板欄位，包括尺寸、邊緣、色彩、膠囊對比及工作區指示器。這些控制沿用原有模組登錄及設定後端儲存。
...
依本次布局及安裝改動要求，不在本機執行測試或建置，也不執行來源／QML／翻譯稽核。驗證交由 push 後已配置的 CI 工作流程執行；有 CI 設定不代表已通過。這次尚未產生真實工作階段截圖，最終視覺驗證仍待完成。
