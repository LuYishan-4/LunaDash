...
新設定參照提供的截圖：頂部保留邊距，左側依序顯示編號工作區、執行中應用程式、目前視窗標題及 CPU／記憶體資訊；啟動器置中，狀態控制與時鐘放在右側。精簡工作區清單至少顯示兩個工作區（僅設定一個時除外），維持連續編號，並在最後一個有視窗的工作區後保留一個備用工作區。第二工作區開窗後出現的第三格，切回第一工作區仍會保留；只切到空白備用工作區不會再新增一格。最小化及其他工作區的視窗仍計入；只改變面板顯示方式，所有已設定的工作區與快捷鍵仍可使用。右側精簡控制中心提供控制、媒體、聲音及系統頁；設定 > 儀表板可切回大型儀表板。設定視窗仍共用原本的標頭與搜尋。
...
啟動器放大原本的 LunaDash 月亮圖標，加入低亮度重點色光暈與外框；保留輔助使用名稱，但滑鼠懸停不再顯示「應用程式」文字提示。「選擇啟動器圖片」開啟既有本機圖片選擇器，「還原 LunaDash 圖標」移除自訂圖片。支援 PNG、JPEG、WebP、GIF 及 SVG，圖片遺失或無法讀取時自動顯示預設圖標。圖片使用選取的本機路徑，請保留該檔案。關閉置中啟動器會將其移到左側；短面板也會移至左側，避免控制項重疊。
