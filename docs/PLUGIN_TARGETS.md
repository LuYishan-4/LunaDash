...
| `window-layout` | `{"windows":[{"id":integer,"x":integer,"y":integer,"width":integer,"height":integer}]}` | `workspace`, `area`, `newWindows`, `layoutMode`; same IDs exactly once, positive sizes, inside work area |

Window layout normally forbids overlap. A native replacement can declare `"layoutMode":"stacking"` to select the host's persistent stacking strategy and permit overlap. The host owns per-window size/position, minimization, focus, maximizing/restoring and workspace membership. `newWindows` identifies windows whose initial placement can be customized; later calls preserve manually edited rectangles. Ordinary tiling continues to own its split tree and shared boundaries. Both strategies pass placements through the same hook and update focus/preview geometry from its validated result.
