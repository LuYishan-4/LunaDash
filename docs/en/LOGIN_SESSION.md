...
External window icons resolve from the Wayland application ID and the installed desktop entry's ID or startup class. Document titles remain visible in tooltips but cannot change an application's icon. Unknown or ambiguous identities use a generic icon. Task buttons retain the selected window ID through status polling and cancel a click if that slot was replaced. Selecting a window restores it if minimized, switches to its workspace and focuses its tile. Each task represents a window and follows its actual focus. Selecting a tiled task maximizes it and hides its workspace peers; selecting it again or pressing Super+F restores the saved tiles.

### Window animations

All normal tiles stay within the work area. Selecting a window uses a subtle fade, and new windows fade in with a short upward movement. Maximizing and restoring fit a retained frame without stretching before revealing the application's new layout; restoring recovers the saved tile arrangement. Closing a window shrinks and fades its last frame, including when the application closes itself.
...
About updates automatically confirm the package transaction, while still requiring system authorization. An authorization request is shown as its own phase before system installation. If the session has no desktop polkit authentication agent, use the installer in a terminal; the background updater will report failure rather than wait for hidden input. A normal terminal installation may still ask `Proceed with installation? [Y/n]`.
...
Dolphin is a core runtime dependency, including when no optional application bundle is selected. It replaces the retired built-in file manager.
