...
The wallpaper plugin uses frame-synchronized, time-based attack/release smoothing (22/140 ms), a logarithmic level scale and adjustable spectrum gain. Base height is 320 px, with a separate Wave amplitude multiplier (default 1.8, range 0.5–3) that also applies to existing installations. Adaptive peak normalization raises quiet playback while preserving differences between frequency bands; it can be disabled. A PCM noise gate suppresses silence. Bar spacing and spectrum gain are configurable. It does not poll MPRIS or use fabricated sine waves; browser audio works too. Silence, mute, or missing samples decay to baseline. A missing helper/server shows a diagnostic and retries every three seconds. Changing the default output restarts the monitor when the shell device list updates. The plugin stays inside the wallpaper Background layer.
...
Audio Wave defaults to 24 bars at each side with 10 px spacing and a 40% clear center. Bass maps to the outer edges and the spectrum tapers toward the center. New side-layout settings apply even when upgrading an older full-width configuration. Disable **Keep the center clear** to restore full-width mode with its own count, spacing and mirror settings.
