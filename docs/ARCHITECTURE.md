...
Native code is colocated beneath six source domains: `service`, `ctl`, `shell`, `config`, `desktop`, and `compositor`. C++ interfaces use adjacent `.hpp` files and implementations use `.cpp`; the low-level C renderer, metrics, and tiling code retain `.h`/`.c` pairs. Entrypoints live in their owning domain, and CMake lists sources explicitly. QML features live under `qml/<feature>/`. Project code is English and uses namespace `LuDash`, except `main`, Qt-generated resource initialization and scanner-generated C protocol symbols.

LunaDash uses a small compositor-owned core-protocol compatibility layer for the seat. Qt 6.9 still initializes `wl_seat` at version 4, while current native clients such as Zed require version 5. On Qt installations that ship usable WaylandCompositor private headers, `CoreProtocolCompositor` creates a LunaDash-owned seat global at version 5 while retaining Qt's pointer, keyboard and touch device implementations; `input.seatProtocolVersion` reports the active version. Distribution packages that omit those private headers fall back to Qt's version-4 seat so the build remains portable. This does not upgrade Qt's separate `wl_data_device_manager` implementation, which is still a distinct compatibility boundary.
