...
Use `Super+H/L` between columns, `Super+J/K` within a grouped column, `Super+Shift+E` to expel, `Super+Ctrl+H/L` to reorder columns, Super plus `+`/`-` to resize, and `Super+C` to center. Merging windows into one column is mouse-only: drag a top-panel application icon onto another one; there is no keyboard grouping shortcut. Every new window opens as its own full-width column and the strip slides horizontally to reveal it. `Super+F` toggles maximize across the compositor `workArea`, whose top edge is below the top panel's `panelExtent`/exclusive area. The frame's top-right minus and × controls provide quick minimize and close actions.
