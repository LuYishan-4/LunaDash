...
LunaDash requires OpenGL 3.3 compatibility or OpenGL ES 3.0 or newer. GLES 2 is not supported.

```sh
QT_QPA_PLATFORM=wayland ./build/lunadash-compositor --graphics opengl
QT_QPA_PLATFORM=wayland ./build/lunadash-compositor --graphics gles
```
...
```sh
env -u MESA_GL_VERSION_OVERRIDE -u MESA_GLSL_VERSION_OVERRIDE QT_QPA_PLATFORM=wayland ./build/lunadash-compositor --graphics opengl
```
